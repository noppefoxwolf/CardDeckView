...
extension ZStackView {
  ...
  private func setupProxyTagHandler<Proxy: ZStackViewProxyProtocol>(
    proxy: Proxy, subviews: SubviewsCollection
  ) {
  ...
  private func updateProxyFrontmostTag<Proxy: ZStackViewProxyProtocol>(
    from subviews: SubviewsCollection, at index: Int?, proxy: Proxy
  ) {
  ...
  private func handleFrontmostTagChange<Tag: Hashable, Proxy: ZStackViewProxyProtocol>(
    newTag: Tag?, subviews: SubviewsCollection, proxy: Proxy
  ) where Tag == Proxy.TagType {
  ...
  private func updateProxyWithAnyHashable<Tag: Hashable>(
    tagType: Tag.Type, subviews: SubviewsCollection, index: Int
  ) {
  ...
  private func findViewIndexWithAnyHashable<Tag: Hashable, Proxy: ZStackViewProxyProtocol>(
    targetTag: Tag, subviews: SubviewsCollection, proxy: Proxy
  ) -> Int? where Tag == Proxy.TagType {
    let tagType = proxy.tagType

    for index in subviews.indices {
      if let tag = extractTagWithReflection(from: subviews, at: index, type: tagType),
        tag == targetTag
      {
        return index
      }
  ...
  private func extractTagWithReflection<Tag: Hashable>(
    from subviews: SubviewsCollection, at index: Int, type: Tag.Type
  ) -> Tag? {
